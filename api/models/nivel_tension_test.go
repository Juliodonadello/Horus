...
func TestTensionGenerador_Validate(t *testing.T) {
	...
	tests := []struct {
		...
		{"Invalid Tension Test", fields{"generador 1", -10.0, time.Now().Unix()}, true},
		{"Invalid Tension Test", fields{"generador 1", 700.0, time.Now().Unix()}, true},
		{"Invalid Time Value Test", fields{"generador 1", 10, time.Now().Unix() + 650}, true},
		{"Invalid Time Value Test", fields{"generador 1", 10, time.Now().Unix() - 650}, true},
	}
